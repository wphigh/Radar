...
class Wphigh_Featured_Content {
	...
	/**
	 * The featured content kit root uri.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 ...
	 *
	 * @see Wphigh_Featured_Content::init()
	 *
	 * @since 1.0.0
	 *
	 * @static
	 ...
	 * @since 1.0.0
	 */
	public static function setup( $domain, $uri ) {
		self::$domain = $domain;
		self::$uri = $uri;
		add_action( 'after_setup_theme', array( __CLASS__, 'add_featured_content_support' ) );
		add_action( 'init', array( __CLASS__, 'init' ), 30 );
	}
	
	/**
	 * A  featured content theme support
	 *
	 * @static
	 * @access public
	 * @since 1.0.0
	 */
	public static function add_featured_content_support() {
		add_theme_support( 'featured-content', array(
			'featured_content_filter' => self::$domain . '_get_featured_posts',
			'max_posts'               => 6,
		) );		
	}	
	 ...
	 * @since 1.0.0
	 *
	 * @return array Array of featured posts object.
	 */
	...
	public static function customize_register( $wp_customize ) {		
		...
		$tag_name = self::get_setting( 'tag-name' );
		if ( empty( $tag_name ) ) {
			$tag_link = _x( 'tag', 'featured content description' , WHTHEME_DOMAIN );
		} else {
			$tag_link = sprintf( '<a href="%1$s">"%2$s" %3$s</a>', admin_url( '/edit.php?tag=' . $tag_name ), $tag_name, _x( 'tag', 'featured content description' , WHTHEME_DOMAIN ) );
		}
		
		$wp_customize->add_section( 'featured_content', array(
			'title'          => __( 'Featured Content', self::$domain ),
			'description'    => sprintf( __( 'Use the %s to feature your posts. You can change this to a tag of your choice; if no posts match the tag, Nothing will be displayed.', self::$domain ), $tag_link ),
			'priority'       => 130,
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'    => __( 'Tag Name', self::$domain ),
			'section'  => 'featured_content',
			'priority' => 20,
		) );
		$wp_customize->add_control( 'featured-content[quantity]', array(
			'label'   => __( 'Quantity', self::$domain ),
			'section' => 'featured_content',
			'priority'=> 25,
		) );		
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'    => __( 'Don&rsquo;t display tag on front end.', self::$domain ),
			'section'  => 'featured_content',
			'type'     => 'checkbox',
			'priority' => 30,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( self::$domain . '-featured-content-customize', self::$uri . '/featured-content-customize.js', array( 'jquery', 'suggest' ), '', true );
	}
